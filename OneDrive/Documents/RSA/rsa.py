...
def encrypt(key, plaintext): ##abc
	text= convertFromASCII(plaintext) ## 979899 
	text= str(text) # "979899"
	blocks=[]
	file_length= len(plaintext) #6
	n=key.n-1
	block_length=math.ceil((n.bit_length())/8)

	for i in range(0,len(str(text)),block_length):
		blocks.append(pow(int(text[i:i+block_length]),key.e,key.n))

	return ciphertext(blocks,file_length,block_length)

# Given the provided rsakey object and ciphertext object plaintext, this will
# perform the RSA decryption. It should return a string. #[111,222]
def decrypt(key, ciphertext):
    plaintext_blocks = []
    for block in ciphertext.c:
        plaintext_block = pow(block, key.d, key.n)
        plaintext_blocks.append(str(plaintext_block))

    plaintext = "".join(plaintext_blocks)
    return convertToASCII(int(plaintext))
